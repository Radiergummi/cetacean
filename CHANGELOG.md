...
All notable changes to Cetacean are documented in this file.
...
- Cross-site request forgery protection covers every write. An origin must be in `server.cors.origins`; `server.public_url` is trusted automatically. A wildcard can't grant writes, so a read-everything deployment must now name its writers
...
- `server.forwarded_headers` names which forwarding headers a trusted proxy writes—`x-forwarded` by default, or `forwarded` for RFC 7239. The family it doesn't name is discarded, so a client can't name its own address
- The authorization server publishes the public key that verifies its access tokens, as a JWK Set at `/oauth/jwks`. Anything checking a token Cetacean issued no longer needs a key that could issue one
...
- The REST API accepts bearer tokens the authorization server issues, so a script or app can authenticate without a browser session—set `oauth.api_tokens` to offer it
...
- Access tokens follow the RFC 9068 `at+jwt` profile. Clients holding an older token refresh automatically
...
- A task no longer sits at `starting` for minutes after it's running, which also left the running count short and anything waiting for a service to settle waiting
...
- JSON Feed responses are labeled `application/feed+json` rather than plain JSON, so a reader can tell they're subscribable
...
- The service and stack lists head their rollout column **Rollout** rather than **Status**, and no longer color a settled rollout green—it reports Docker's update state, not whether the service is running
...
- Stacks in the topology view can be told apart by color again
...
- `find` and `describe` return links to the resources they're about
...
- Cetacean exports traces to an OpenTelemetry collector via `CETACEAN_OTEL_ENDPOINT`, joining a calling agent's trace where there's one
...
### Changed
- **Breaking:** the MCP server speaks protocol revision 2026-07-28 only. Older revisions are refused with an error naming the version to use—update your client if it can't negotiate it
- **Breaking:** an AI agent receives a compact description of a resource rather than raw Docker output. Listing and searching are now one `find` tool, with a new `describe` for detail; raw records are still available on request
...
- MCP authorization responses identify the issuer (RFC 9207), so a client can't be tricked into redeeming a code at the wrong one
...
- MCP clients registering dynamically declare whether they're a native or web application, and are held to the redirect URIs that implies
...
- An MCP client whose permissions match no grant no longer receives change notifications, which leaked the timing of changes it can't see
...
- The bundled monitoring stack applies its own Prometheus configuration; it was mounted where Prometheus doesn't read it, so no exporters were ever discovered
...
- Live log tail reconnects on its own, resumes where it left off, and shows what it's doing; at the server's connection limit it counts down rather than showing a cryptic error
...
- Log reads say when they couldn't reach the start of the requested window, or when a search ran out of budget before the older lines
- Searching within a live log, and the change timeline, now report how far back they can answer for
- Browsers no longer reconnect in lockstep after a restart, which made them collide and synchronize harder each round
...
- Breadcrumbs on a resource in a stack lead through the stack, and a resource that belongs to none no longer shows a stack prefix that isn't there
...
- Editing a service's ports by reading them and writing them back no longer destroys the published port. Any section given a field it doesn't have is now refused, naming it
...
- A service running an image from a private registry on a port no longer links to a Docker Hub page that doesn't exist
...
- **Breaking (MCP):** a recommendation that can be acted on names the tool that does it, rather than an HTTP route an agent can't call
...
- An MCP client whose permissions match no grant no longer sees tools it can't use
...
- MCP `tools/list` hides write tools the caller has no grants for, and change notifications skip clients that can't read the affected type
...
- The MCP consent page no longer surfaces raw fetcher errors, and isn't cached, so a back-button replay can't surface another user's session
...
- Pinned `hono` ≥ 4.12.18, `ip-address` ≥ 10.1.1, `postcss` ≥ 8.5.10 in the dashboard's transitive (dev-only) dependencies
- Updated the marketing site's dependencies via `npm audit fix`, clearing the critical `protobufjs` arbitrary-code-execution advisory (GHSA-xq3m-2v4x-88gg) and the bundled `dompurify`/`astro` advisories
...
- Manual resync button next to the live-connection indicator and `POST /-/resync` endpoint for forcing a full refetch when the cache appears stale
...
### Deprecated
- `CETACEAN_AUTH_HEADERS_TRUSTED_PROXIES`—use `CETACEAN_TRUSTED_PROXIES` instead; slated for removal in v1
...
### Added
- Sub-path deployment: serve Cetacean under a configurable URL prefix (for example `/cetacean/`) via `CETACEAN_BASE_PATH` environment variable, `--base-path` flag, or `base_path` TOML config
...
- Release Docker image health check not detecting Docker connectivity issues
...
- Health check timeline is keyboard-accessible and scrollable on small screens
...
- Copy-to-clipboard buttons on container configuration and health check values
- Docker Swarm template expressions (such as `{{.Node.Hostname}}`) rendered as human-readable badges
...
- Health check editor on service detail page (view and edit health check configuration)
- Interactive health check timeline on task detail page
...
- Node metrics showing when node-exporter targets aren't available
...
### Added
- Tasks list page (the "Failed Tasks" card on the homepage no longer returns 404)
...
- Stale fetch requests not canceled when navigating away from detail/search pages
...
### Added
- TOML config file and command-line flags as alternative to environment variables
...
### Fixed
- Log fetches for multi-replica services reduced from about 10 seconds to about 2 (idle timeout on stream parser)
