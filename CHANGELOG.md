...
- The API documentation is compressed and cacheable like every other endpoint. `/api` serves the largest document this API has and neither it nor the API playground's script carried a validator or a content coding, so both were re-downloaded in full on every visit
