...
- `CETACEAN_MCP_SIGNING_KEY` is now a root secret that the token and consent keys are each derived from, rather than being used directly as both. Generate one with `openssl rand -hex 32` — a long value that isn't hex or base64 still works, but is now warned about at startup
