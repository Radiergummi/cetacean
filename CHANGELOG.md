...
- An MCP signing key shorter than 32 bytes is now refused at startup instead of being used. HMAC-SHA256 accepts a key of any length, so a one-character key signed every token the MCP server issued and nothing anywhere complained. Leaving the key unset is unaffected and still generates a random one
