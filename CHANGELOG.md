...
### Fixed
- The dashboard keeps its icons while the Docker daemon is unreachable. Anything outside the API was answered with an error document as soon as the connection to Docker dropped, so the shell loaded but none of the files it pulls did — the favicon, the touch icon, the web app manifest — leaving the tab blank exactly when something had gone wrong. Those files come off the embedded frontend and read no cluster state; only the endpoints that do still refuse
