...
- The API documentation, the API playground's script and the JSON-LD context are compressed and cacheable like every other endpoint. `/api` serves the largest document this API has, and none of the three carried a validator or a content coding, so each was re-downloaded in full on every visit
