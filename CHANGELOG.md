...
- MCP access tokens are signed with ES256 instead of HMAC. Clients re-authenticate once on upgrade; if you run more than one replica, stop them all before starting the new version
- `CETACEAN_MCP_SIGNING_KEY` is now a root secret that the token and consent keys are each derived from, rather than being used directly as both. Generate one with `openssl rand -hex 32` — a value that is not 32 bytes of hex or base64 still works, but is now warned about at startup
