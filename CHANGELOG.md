...
- Cross-site request forgery protection now covers every write. A state-changing request a browser reports as coming from another origin is refused unless that origin is listed in `server.cors.origins` — one setting deciding both which origins may read the API and which may write to it. A deployment that sets `server.public_url` has that origin trusted for writes automatically, so a dashboard behind a proxy never has to name itself. It matters most in the authentication modes whose credential a browser attaches by itself: a Tailscale identity, a client certificate, or a header a proxy adds, none of which a session cookie's `SameSite` rule protects. Reads are untouched, and so is every client outside a browser. A wildcard is the one value that cannot serve both halves — `*` is not an origin — so a deployment that allows every origin to read must now name the ones allowed to write, browser-based MCP clients included
...
- Cetacean publishes a catalogue of the APIs it serves at `/.well-known/api-catalog` (RFC 9727), and every response points at it. A client that knows nothing but the address can read the catalogue and find the OpenAPI description, the API reference, the JSON-LD context and the health probe — and, when MCP is turned on, that there is an MCP server at `/mcp` and where its authorization metadata lives. The catalogue lists only what the process is actually running, so it never points at an endpoint that would answer 404
- The cluster is searchable from the browser's address bar. Cetacean serves an OpenSearch description at `/opensearch.xml` and the dashboard advertises it, so a browser that supports the convention can be taught a keyword — type it, then a service name, and land on the search results. The description also offers the Atom and JSON forms of the same search, for a feed reader or a script
- Any list can be downloaded as a spreadsheet. Add `.csv` to a list URL — or ask for `text/csv` — and services, nodes, tasks, stacks, configs, secrets, networks, volumes, the change history and the recommendations come back as a CSV file with a header row, named for what it holds and the day it was taken. The columns are the ones the dashboard's own tables show rather than the whole Docker object, and search, filters and sorting apply exactly as they do elsewhere, so a filtered view can be handed to somebody who would rather read it in a spreadsheet. One thing differs from the JSON: a download that asks for no particular page gets every row, since a file truncated at a default gives no sign inside itself that it was truncated
- The dashboard is installable as an app. It ships a web app manifest with icons, so a browser can offer to install it and open it in its own window rather than a tab, and the browser chrome now takes its colour from the dashboard's own background in both light and dark themes instead of framing the page in the opposite one
- The MCP authorization server now publishes the public key that verifies its access tokens, as a JWK Set at `/oauth/jwks` advertised by `jwks_uri` in its metadata. Anything that needs to check a token Cetacean issued can now do so without holding a key that could issue one

### Changed
- MCP access tokens now follow the standard JWT access token profile (RFC 9068). Each is tagged `at+jwt`, and a token that is not tagged that way is refused — which is what lets a client be certain an ID token can never be accepted where an access token belongs. A client holding a token from an older build is told once that it is invalid and quietly fetches a new one with the refresh token it already has, so there is nothing to do on upgrade and nobody has to authorize again
...
- An endpoint with only one thing to return no longer refuses a request for naming a format it has never heard of. Cetacean rejected an unrecognised `Accept` header before it knew which URL was being asked for, so a health probe, the sign-in identity endpoint or the API playground's script — none of which offer a choice of format — answered `406` to a client that happened to send an unusual one. They now answer as they otherwise would, which is what the HTTP specification asks of an endpoint that negotiates nothing. Write requests are in the same position and behave the same way. Endpoints that genuinely do offer a choice still refuse a format they cannot produce
- MCP access tokens are signed with ES256 instead of HMAC. Clients refresh once on upgrade; if you run more than one replica, stop them all before starting the new version
- **Breaking:** `CETACEAN_MCP_SIGNING_KEY` is now a root secret that the token and consent keys are each derived from, rather than being used directly as both, and it must be 32 bytes of hex or base64 — Cetacean refuses to start on anything else, where it previously accepted any value of 32 characters or more. Generate one with `openssl rand -hex 32`. The reason for the tightening is the change above: the public half of the signing key is now published and needs no authentication to fetch, so a passphrase used as the root could be guessed offline from it. Leaving the setting unset is unaffected — a key is generated at startup as before

### Fixed
- Everything that does not describe the cluster keeps working while the Docker daemon is unreachable. Anything outside the API was answered with an error document as soon as the connection dropped, so the shell loaded but none of the files it pulls did — the favicon, the touch icon, the web app manifest — leaving the tab blank exactly when something had gone wrong. The API catalogue and the OpenSearch description went with them, which is the moment a client probing an unresponsive server most needs them, and so did `/profile`, so you could not see who you were signed in as. Only the endpoints that actually read the cluster still refuse
- Asking an endpoint for a format it does not serve now says so instead of answering with JSON. Requesting `/services` as GraphML, as a Graphviz graph or as a JSON Graph — formats only `/topology` produces — returned a JSON listing, which is the one answer a client that asked for a graph cannot use. Each endpoint now decides for itself what it can produce, which is also what lets a document with a single format be fetched by naming that format: the OpenSearch description, the API catalogue and the JSON-LD context each answer a client asking for exactly what they serve
- Relabelling a node now needs the same operations level over the API as it does over MCP. `PATCH /nodes/{id}/labels` was gated at level 3, alongside draining a node and demoting a manager, while MCP's `update_node_labels` sat at level 2 — so at level 2 an agent could relabel a node while the dashboard refused the same edit. Labels are a placement edit and now sit at level 2 on both, which is what lets a deployment allow relabelling without also allowing a manager to be demoted
- The API documentation, the API playground's script and the JSON-LD context are cacheable like every other endpoint, and the large ones are compressed. None of the three carried a validator, so each was re-downloaded in full on every visit even though none of them changes between releases. `/api` serves the largest document this API has and now compresses too; the context document is small enough that it never needed to
- Documents that never change are now compressed once rather than on every request. The playground's script is 3.7 MB and its route needs no authentication, so re-compressing it per request let anyone spend real server CPU on a tiny request; the attribution documents behind `/-/licenses` and `/-/notices` had the same shape
- Feed alternate links in `Link` headers now carry only the parameters the feed they point at actually reads, matching the links inside the feed itself. An arbitrary query parameter was echoed back in the header
...
- An Atom feed no longer identifies itself by one host while every link inside it names another. Behind a reverse proxy with no `server.public_url` set, a feed's identifier was built from the address Cetacean was reached on inside the cluster, while its links carried the address readers actually see; both now come from the same place. A feed reader may treat such a feed as new once after upgrading — setting `server.public_url` is what keeps a feed's identity stable, since nothing derived from a request can be
