...
### Fixed
- A format an `Accept` header rules out with `;q=0` is no longer served anyway. Cetacean read the weight when ranking formats but not when deciding whether one was allowed at all, so `Accept: text/html;q=0` was answered with HTML; such a request is now refused with `406`, as the HTTP specification requires
- Deep links into a service whose name contains a dot now open instead of answering 404. Docker permits dots in a name, so `/services/web.api/logs` — any tab of any such service — was read as a feed reader probing for a feed, because any dot in the middle of an address counted. Only the extensions Cetacean actually serves are read that way now
- An address matching nothing now answers `404` instead of handing back the dashboard, whenever the client said it could not use a web page. Adding `.json`, `.atom` or any other extension to such an address returned the dashboard's HTML with a `200`, and so did a request whose `Accept` header asked for JSON — a script was told it had succeeded and given a web page, and every registered well-known URI ending in `.json` was out of reach. The refusal is now a standard problem document, as is the one for a discovery probe carrying the extension mid-address, which was plain text naming no format at all. A client that expresses no preference is unaffected, so the dashboard's own addresses, its assets and its not-found page all behave exactly as before
