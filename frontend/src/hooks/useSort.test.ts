...
describe("useSort", () => {
  ...
  it("sorts by default key ascending", () => {
    const { result } = renderHook(() => useSort(items, accessors, "name"));
    expect(result.current.sorted.map((i) => i.name)).toEqual(["Alice", "Bob", "Charlie"]);
    expect(result.current.sortDir).toBe("asc");
  });

  it("sorts numerically", () => {
    const { result } = renderHook(() => useSort(items, accessors, "age"));
  ...
  it("handles undefined values", () => {
    ...
    const { result } = renderHook(() => useSort(data, accessors, "name"));
