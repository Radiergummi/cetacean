...
import {formatTime, isJSON, LEVEL_BAR, LOG_ROW_HEIGHT_ESTIMATE, LOG_VIRTUAL_THRESHOLD, logLineKey} from "./log-utils";
        ...
        >
            ...
            {showAttrs && (
                <td
                    ...
                    title={
                        line.attrs?.taskId
                            ? `Filter by task ${line.attrs.taskId.slice(0, 12)}`
                            : undefined
                    }
                >
                    {line.attrs?.taskId && onTaskFilter ? (
                        ...
                        <span className="text-muted-foreground/60">{line.attrs?.taskId?.slice(0, 8).trim()}</span>
                    )}
                </td>
            )}
            <td
