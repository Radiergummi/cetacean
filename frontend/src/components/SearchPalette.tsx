...
function StateOrb({state}: { state: string }) {
    ...
    return <span className={`inline-block w-2 h-2 rounded-full shrink-0 ${color}`} title={state} />;
}
...
export default function SearchPalette({onClose}: { onClose: () => void }) {
    ...
    const goTo = useCallback(({result: {id}, type}: FlatItem) => {
                            ...
                            <li key={type}>
                                <section>
                                    ...
                                    {items.map(({index, result}) => (
                                        <button
                                            key={`${type}-${result.id}`}
                                            className={`flex w-full justify-between items-center gap-2 px-3 py-1.5 text-sm text-left cursor-pointer ${
                                        ...
                                        >
                                            ...
                                            {result.detail && (
                                                <span className="truncate text-muted-foreground text-xs">{result.detail}</span>
                                            )}
                                        </button>
                                    ))}
                                </section>
