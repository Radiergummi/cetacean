  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <span className="text-2xl font-semibold tabular-nums group-data-[status=amber]:text-status-warning group-data-[status=green]:text-status-ok group-data-[status=red]:text-status-danger">
          {primary}
        </span>
        {delta != null && delta !== 0 && (
          <span
            data-negative={delta < 0 || undefined}
            className="flex items-center gap-0.5 text-xs font-medium text-status-danger data-negative:text-status-ok"
          >
