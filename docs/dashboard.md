...
- **Recommendation badge:** bulb icon with count; amber when there are active recommendations
...
The palette also supports **actions:** type an action name (e.g., `scale`, `restart`, `drain`) to trigger multistep
...
Results show live state: a spinning orb for updating resources, colored dots for running/failed/completed.
...
- **Action buttons:** scale, update image, rollback, restart (gated
  by [operations level](configuration.md#operations-level))
- **Configuration editors:** inline editors for environment variables, labels, resources (CPU/memory), health check,
...
- **Health check timeline:** color-coded status history when health checks are configured
...
- **Sizing recommendations:** per-service banners when resource issues are detected
...
The metrics console (`/metrics`, `g m`) is a PromQL query builder for running adhoc Prometheus queries. Results are
...
- **Search:** substring or regex, with match counter and next/previous navigation (`Enter`/`Shift+Enter`)
    ...
    - `.*` toggles regex mode
- **Pin lines:** click to pin up to three lines to the top of the viewer
...
"Load older" appears at the top when more history is available.
...
**Brush to zoom:** click and drag horizontally to select a time range. The chart zooms in and the URL updates.
...
Charts use a 10-color CVD-safe palette that adapts to light and dark themes. The same resource always gets the same
color across pages.
...
The Recommendations page (`/recommendations`, `g r`) shows cluster health recommendations across four domains: sizing,
config hygiene, operational health, and cluster topology. The dashboard includes a severity summary card, and service
detail pages show per-service banners. See [docs/recommendations.md](recommendations.md) for the full reference.
...
These preferences are saved in the browser’s localStorage and survive page reloads:
