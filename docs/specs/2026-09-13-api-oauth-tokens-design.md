...
- Both PRM documents under a base path, and the `WWW-Authenticate` from each transport naming its
  own.
- ~~A restart with the old `mcp-tokens.json` present keeping every grant alive.~~ Moot: the
  legacy fallback was dropped outright in the extraction, so there is no path left to cover.
