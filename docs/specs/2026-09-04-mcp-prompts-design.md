...
The text lives in `internal/mcp/prompt_text.go`, which is its source of truth;
it is not reproduced here. An earlier revision of this document quoted all six
prompts verbatim and drifted from the code twice in a single branch, including
retaining a Docker state value that Docker never emits after the code was
corrected. The five rules above are the spec-level content; the wording is
implementation.
...
mcp-go's `handleComplete` dispatches straight to the registered provider with
no prompt-filter check, so #170 must itself filter completions by the same
per-identity predicate (`filterPromptsForIdentity`/`toolVisibilityFor`) —
otherwise `completion/complete` would answer for a prompt the caller cannot
see, undoing the disclosure property this branch establishes.
