...
|---|---|---|
| Success | 307 | — |
| Any pair resolves to nothing, or to something unreadable | 404 | `API016` |
| The edge does not hold between two named resources | 404 | `API016` |
| A name matches more than one readable resource | 409 | `API015` |

`API016` ("Unresolvable Path") is one code with a precise detail rather than two
...
`API016` belongs to 2b and should not be registered before it: an error code in
the catalog that nothing can produce is a promise to a reader of
`/api/errors`. 2a's own failures are all covered by `API015` and the handlers'
existing 404s.

`API015` already exists and already lists every candidate ID. Because that list
...
`API015` ambiguity report, the read-grant gate, and the HTML exemption.
...
- An edge that does not hold answers `404` `API016`, naming both resources.
