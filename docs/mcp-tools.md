...
**Arguments** — `type`, `query`, `state`, `stack`, `node`, `image`, `label`, `limit`, `offset`, `raw`
...
**Arguments** — `type` (singular, required), `id` (required), `raw`
...
**Arguments** — exactly one of `service`, `task`, `stack`, `cluster`; plus `contains`, `tail`, `since`, `level`
...
**Arguments** — `since`, `until`, `types`, `resource`, `limit`

#### `get_topology`

Projects the cluster as a graph in three views.

**Arguments** — `view` (`network`, `placement`, `drain-impact`), `node`
...
**Arguments** — `target` (required), `id`, `metric`, `range`, `top`, `by`
...
Returns the same findings as `cetacean://recommendations`, with totals counting what the caller may read. A
...
**Arguments** — `service` (required), `timeout`
...
**Arguments** — `name`, `data`, `encoding`, `labels`
