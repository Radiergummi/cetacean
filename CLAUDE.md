...
- **`api/write_middleware.go`** — `requireWrite` middleware. Currently a pass-through placeholder for future RBAC (role-based access control). Will check `identity.Groups` against allowed roles once authorization is implemented.
