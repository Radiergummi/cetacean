...
- **`mcp/tasks.go`** — Tasks extension (2026-07-28). `serviceConverged` reports a service converged once its running task count matches its desired replicas and no rolling update is in flight; `awaitConvergence` polls the cache until that holds; `awaitIfTask` runs the wait **only** when `req.Params.Task != nil`, so a plain `tools/call` still returns as soon as Docker accepts. The four converging tools (`scale_service`, `update_service_image`, `rollback_service`, `restart_service`) stay ordinary `AddTool` registrations carrying `WithTaskSupport(TaskSupportOptional)` — `AddTaskTool` would make mcp-go refuse every synchronous call. `awaitIfTask` detaches the context (`context.WithoutCancel`) because mcp-go runs the task on a goroutine holding the already-cancelled HTTP request context; `convergenceTimeout` is the real bound, and `tasks/cancel` therefore cannot interrupt the wait. `registerTools` converts a handler error into a real error (not a tool-error result) when the call is task-augmented, so a refused mutation reports `failed` rather than `completed`.
