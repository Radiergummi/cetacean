...
- **`widgets/`** — MCP Apps widget bundles, a second Vite target (`vite.config.widgets.ts`) rooted at `src/widgets/` emitting one self-contained HTML file per widget into `frontend/dist-widgets/<name>/index.html` via `vite-plugin-singlefile` (an app resource has no base URL, so nothing may load from a relative path). `bridge.ts` wraps `@modelcontextprotocol/ext-apps/react`: `useCetaceanHost` (handshake + host theme, and `app.callServerTool`, *not* `callTool`) and `useToolData` (request-keyed, so a superseded answer cannot overwrite the current one). Widgets read data only through MCP tools, never Cetacean's HTTP API, so every read stays on the ACL-checked path. The `table` widget renders a `list_resources` result: `ResourceTable` is the pure, tested half (search, sort, "showing N of M"), `columns.ts` maps each resource type to accessors over the **raw Docker field names** (`Spec.Name`, `Description.Hostname`) with a name/ID fallback for a type it has not been taught, and `TableWidget` is only the `useToolData` wiring. Build with `npm run build:widgets`; `make build`, the Dockerfile and CI all run it, and CI's `frontend-dist` artifact carries `dist` and `dist-widgets` both.
