...
- **`api/write_middleware.go`** — `requireWrite` middleware. Validates `Content-Type` against allowed types; returns 415 for unsupported media types.
...
## Key Conventions
- Most API endpoints are read-only (GET). Write operations use: PUT for idempotent updates (scale, image, availability), POST for non-idempotent actions (rollback, restart), PATCH for partial updates (env vars, labels, resources), DELETE for removal (tasks). All write endpoints go through `requireWrite` middleware which enforces `Content-Type: application/json` (or `application/json-patch+json` / `application/merge-patch+json` for PATCH).
