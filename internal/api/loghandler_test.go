...
func TestHandleServiceLogs_JSON(t *testing.T) {
	...
	h := NewHandlers(c, &mockLogStreamer{data: frames.Bytes()}, closedReady(), nil, nil)
...
func TestHandleServiceLogs_JSON_NotFound(t *testing.T) {
	c := cache.New(nil)
	h := NewHandlers(c, &mockLogStreamer{}, closedReady(), nil, nil)
...
func TestHandleServiceLogs_JSON_Empty(t *testing.T) {
	...
	h := NewHandlers(c, &mockLogStreamer{data: nil}, closedReady(), nil, nil)
