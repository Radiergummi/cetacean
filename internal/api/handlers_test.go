...
func TestHandleHealth(t *testing.T) {
	h := NewHandlers(cache.New(nil), nil, nil, nil, closedReady(), nil)
...
func TestHandleReady_NotReady(t *testing.T) {
	...
	h := NewHandlers(cache.New(nil), nil, nil, nil, ch, nil)
...
func TestHandleClusterMetrics_WithPrometheus(t *testing.T) {
	...
	h := NewHandlers(c, nil, nil, nil, closedReady(), NewPromClient(prom.URL))
...
func TestHandleMonitoringStatus_PrometheusUnreachable(t *testing.T) {
	...
	h := NewHandlers(cache.New(nil), nil, nil, nil, closedReady(), NewPromClient("http://127.0.0.1:19999"))
...
func TestHandleStackSummary_PrometheusDown(t *testing.T) {
	...
	h := NewHandlers(c, nil, nil, nil, closedReady(), NewPromClient("http://127.0.0.1:1"))
	req := httptest.NewRequest("GET", "/stacks/summary", nil)
	w := httptest.NewRecorder()
	h.HandleStackSummary(w, req)
